...
const buildUserResponse = (user, doctorProfileId = null) => ({
  ...
  // Doctor profile _id — used by patients to route video calls to this doctor.
  // Different from user._id (User model) because Doctor is a separate model.
...
const login = asyncHandler(async (req, res) => {
  ...
  // If user is a doctor, fetch their Doctor profile record.
  // We need the Doctor model _id (doctorProfileId) so the frontend can:
  //  1. Register the video socket with this ID
  //  2. Allow patients to route incoming calls correctly
